function seahawk = convert_seaHawk_flowThrough_to_mat_deploy2(datDir,dates,arcDir);
%
% USAGE: seahawk = convert_seaHawk_flowThrough_to_mat_deploy2(datDir,dates,arcDir);
%
% inputs:
...
% note: there were issues with the GPS-stream, so track lines from the vessel's
%       navigation unit was saved for, as of 05/10/2024, only 02/15/2024.
%       As a result, I've heavily modified this code to ingest the new GPS data.
...
for jj = 1:nd
    ...
    flowFiles(ismember({flowFiles.name},{'.','..'}) | cell2mat({flowFiles.isdir})) = [];
    flowFiles(endsWith({flowFiles.name},'.mat') | endsWith({flowFiles.name},'.xlsx')) = [];
    ...
    % split files
    if sum(isNMEA)~=sum(isRVSH), fprintf('missing at least one NMEA/RVSH file pair; \n continuing anyway...\n'), , end
    ...
    for kk = 1:nf
        ...
        fid      = fopen(NMEAfile);
        NMEAdata = textscan(fid,['%s %s %f %f',repmat(' %*s',1,7)],'delimiter','\t');
...
% $$$         lat0 =lat0(1:N) ; lon0= lon0(1:N);
        ...
        %
        timeString = NMEAdata{2};
        %
        NMEA(jj,kk).date_vec  = datevec(timeString);% [ year, mon, day, hour0, min0, sec0 ];
        NMEA(jj,kk).timeUTC   = datenum(timeString);%datenum([ year, mon, day, hour0, min0, sec0 ]);
        NMEA(jj,kk).time_zone = [];
        NMEA(jj,kk).latitude  = NMEAdata{3};% lat0;
        NMEA(jj,kk).longitude = NMEAdata{4};% lon0;
...
% $$$         end
        ...
        isTime   = ~isnat(dateTime);
        SENSOR(jj,kk).timeUTC    = convertTo(dateTime(isTime),'datenum')+D{3}/86400;
        SENSOR(jj,kk).temperature= D{23}(isTime);
        SENSOR(jj,kk).salinity   = D{15}(isTime);
        SENSOR(jj,kk).turbidity  = D{19}(isTime);
        SENSOR(jj,kk).ChlA       = D{5}(isTime);
        SENSOR(jj,kk).fDOM       = D{8}(isTime);
        SENSOR(jj,kk).O2sat      = D{11}(isTime);
        SENSOR(jj,kk).O2mg_L     = D{13}(isTime);
        SENSOR(jj,kk).pH         = D{22}(isTime);        
        SENSOR(jj,kk).pressure   = D{14}(isTime);
    ...
    if nf>1
        ...
        % if SENSORstart > NMEAend or vice-versa skip that file
        indNMEA = indNMEAstart;        
        badNMEA = (NMEAstartTimeUTC(indNMEAstart) > SENSORendTimeUTC) |...
                  (NMEAendTimeUTC(indNMEAstart)   < SENSORstartTimeUTC);% |...;
        indNMEA = indNMEA.*~badNMEA;
    ...
    for kk = 1:nf
        ...
        if ii
            ...
            for ll = 1:length(varsNMEA)
                if ~isempty( eval(['NMEA(jj,ii).',varsNMEA{ll}]) )
                    eval(['temporary(jj,kk).',varsNMEA{ll},' = interp1(timeNMEA(goodNMEA)*86400,NMEA(jj,ii).',varsNMEA{ll},'(goodNMEA,:),timeINT);']);
                else
                    eval(['temporary(jj,kk).',varsNMEA{ll},' = [];'])
                end
            end
        else
            ii        = find(~ismember(1:nf,indNMEA));
            if length(ii)>1
                fprintf('error combining NMEA and SENSOR structures, multiple missing NMEA files')
                return
            end
            timeINT   = timeSENSOR;
            timeINT    = round( timeINT*86400 );
