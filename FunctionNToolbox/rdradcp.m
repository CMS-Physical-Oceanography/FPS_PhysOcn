...
%
%  This current version does have some handling of VMDAS, WINRIVER, and 
%  WINRIVER2 output files, but it is still 'beta'. There are (inadequately 
%  documented) timestamps of various kinds from VMDAS, for example, and 
%  caveat emptor on WINRIVER2 NMEA data.
%
...
%
%   28/Dec/2005 - redid code for recovering from ensembles that mysteriously change length, added
...
%   23/Aug/2006 - ouput some bt QC stiff
...
%   29/Oct/2017 - vector averages for some VMDAS speed/direction variables
%                 also pitch/roll/heading from VMDMAS nav block
%    4/Jan/2018 - VmDas 1.43 and later modifications (also fixed bug that didn't
%                 properly handle missing bottom-track velocities)
%   28/Sep/2021 - Sentinel V firmware 66 support
...
fprintf(' File comes from %s (firmware %5.2f), source of data from: %s\n',cfg.name,cfg.prog_ver,cfg.sourceprog);
fprintf(' %d Beams tilted at %d degrees in a %s arrangement\n',cfg.numbeams,cfg.beam_angle,cfg.beam_pattern);
fprintf(' Acoustic Frequency: %d kHz\n',cfg.beam_freq);
fprintf('\n');
fprintf(' Cell size: %5.2f, # cells: %d, deployed facing %s\n',cfg.cell_size,cfg.n_cells,cfg.orientation);
fprintf(' Pings per ensemble: %d\n',cfg.pings_per_ensemble);
fprintf(' Data in %s coordinates\n',cfg.coord_sys);
fprintf('\n');
...
for k=1:n
  ...
  if ~isstruct(ens) % If aborting...
    adcp.rerunparameter=(k-1)*num_av;
  ...
  zz=exp(i*ens.heading*pi/180);              % Vector averaging - RP OC=ct/2017
  adcp.heading(k)     =angle(mean(zz))*180/pi;
 % fprintf('%f %f\n',ens.heading(1),adcp.heading(k));
  ...
  if ischar(vels)                                  % added despiking for botom velocities RP 10/13
 ...
 %%%   disp(['--' datestr(adcp.nav_mtime(k),'HH:MM:SS')]);
   ...
   case 'VMDAS'
     ...
     zz=ens.speedmadegd.*exp(i*pi/180*(90-ens.direcmadegd));  % Don't average speed and angles
     adcp.nav_speedmadegd(k)=abs(nmean(zz));                  % Instead do vector average - RP Oct/2017
     adcp.nav_direcmadegd(k)=90-angle(nmean(zz))*180/pi;
...
% This is what I know about firmware versions from scouring RDI manuals:
%
% version - instrument
% 4,5 - BB-ADCP
% 8,9,16 - WH navigator
...
if fix(cfg.prog_ver)==8 || fix(cfg.prog_ver)==10 || fix(cfg.prog_ver)==16 ...
  ...
  %  cfg.serialnum      =fread(fd,1,'int64');
    nbyte=nbyte+8; 
  end
  ...
  if cfg.prog_ver>=16.27   % Added bytes for REMUS, navigators, and HADCP (not meaningful for others)
...
elseif  fix(cfg.prog_ver)==47 || fix(cfg.prog_ver)==66
    fseek(fd,8,'cof'); % spare
    cfg.sysbandwidth  =fread(fd,2,'uint8');
    cfg.syspower      =fread(fd,1,'uint8');
...
% SOURCE is 0 regular file
%           1 - VMDAS (before v1.43)
%           1.1 - VMDAS (v1.42 and after)
%           2 - WINRIVER
%           3 - WINRIVER II
 ...
 % to decode the info in each block and return this to the user.
   ...
   %   fprintf('frm=%7.4f  NCELLS = %d\n',cfg.prog_ver,cfg.n_cells);
     case {'0001'}
      ...
      fprintf(' Is this a VMDAS file with two types of ping? If so, re-run with the ''type'' option specified\n');
      nbyte=2;
...
%%fprintf('\n k %8.3f',ens.slatitude(k));
      ...
      ens.bt_vel(:,k)  =fread(fd,4,'int16')*.001; % m/s  Determined this should be scaled by 1000 Sep/2021
      ii=ens.bt_vel(:,k)==-32768*.001;                 % Change baddata to NaN 10/13
      if any(ii),ens.bt_vel(ii,k)=NaN;  end
...
% block. 
    ...
    case {'2000'}  % Something from VMDAS.
      ...
      ens.nmtime(k)     =mtime+fread(fd,1,'uint32')/1e4/86400;
       ...
       pitch=fread(fd,1,'int16')*cfac2;   % Pitch here (not in header?)             
       roll=fread(fd,1,'int16')*cfac2;     % Roll here (not in header?)             
       heading=fread(fd,1,'int16')*cfac2;  % Heading here (not in header?) 
      
      if pitch~=0 ens.pitch(k)=pitch;  end %  overwrite only if nonzero numbers are here.   
      if roll~=0 ens.roll(k)=roll;   end  %         
      if heading~=0 ens.heading(k)=heading; end %               
      fseek(fd,10,'cof');
      nbyte=2+76;
      
      % With VmDas 1.43 another 14 bytes were added here.
      if SOURCE==1.1 || fix(cfg.prog_ver)==66
        fseek(fd,14,'cof');
        nbyte=nbyte+14;
      end       
...
% These are for V-series ADCPs (47.xx firmware)
    ...
    case {'7002'}    % Advanced system diagnostics
      ...
      nbyte=2+14;
      if fix(cfg.prog_ver)==66
         fseek(fd,2,'cof');   % Skip them all
         nbyte=nbyte+2;
      end   
...
%%   fprintf('#bytes is %d, original offset is %d\n',nbyte,byte_offset);
    ...
    if n<length(hdr.dat_offsets)
      ...
      if hdr.nbyte-2~=byte_offset    
        ...
        if strcmp(id,'2000') && SOURCE==1
           fprintf(' Seems to be VmDas version later than 1.43 - assume longer Binary Nav block\n');
           SOURCE=1.1;
         end      
      end
